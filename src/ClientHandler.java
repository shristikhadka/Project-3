...
public class ClientHandler implements Runnable{
    final private  SocketChannel serveChannel;
    ...
    public void run() {
        try {
            ...
            int bytesRead = serveChannel.read(commandBuffer);
            ...
            switch (command) {
                case 'L' -> { // LIST
                    ...
                    String reply = (files != null && files.length > 0)
                            ? String.join("\n", files)
                            : "No files found.";
                    serveChannel.write(ByteBuffer.wrap(reply.getBytes()));
                ...
                case 'R' -> { // RENAME
                    ...
                    serveChannel.read(oldNameBuffer);
                    oldNameBuffer.flip();
                    ...
                    serveChannel.read(newNameBuffer);
                    newNameBuffer.flip();
                ...
                case 'U' -> { // UPLOAD
                    ...
                    int uploadBytesRead;
                    while ((uploadBytesRead = serveChannel.read(contentBuffer)) != -1) {
                        contentBuffer.flip();
                ...
                case 'W' -> { // DOWNLOAD
                    ...
                    if (!downloadFile.exists()) {
                        ...
                        statusBuffer.flip();
                        serveChannel.write(statusBuffer);
                        ...
                        while ((downloadBytesRead = fis.read(chunk)) > 0) {
                            ...
                            contentBuffer.flip();
                            serveChannel.write(contentBuffer);
                        }
